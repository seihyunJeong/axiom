          ...
          {nodes.map(node => (
            <Item>
              <a key={node.id} href={node.link} target="_blank" rel="noopener noreferrer">
                <ItemContent>
                <Logo src={`/partner/${node.id}.png`} onMouseOver={() => { console.log(globalThis.document.images); console.log('mouse over'); }} onMouseOut={() => { src=`/partner/${node.id}.png`; console.log("mouse out");}}/>
