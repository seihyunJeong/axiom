          ...
          {nodes.map(node => (
            <Item>
              <a key={node.id} href={node.link} target="_blank" rel="noopener noreferrer">
                <ItemContent>
                <Logo src={`/partner/${node.id}.png`} onMouseOver={(this) => { this.src=`/partner/${node.id}_hover.png`; console.log('mouse over'); }} onMouseOut={(this) => { this.src=`/partner/${node.id}.png`; console.log("mouse out");}}/>
